...
async def book_training(user_id, training_id):
    async with async_session() as session:
        if await session.scalar(select(Booking).where(Booking.user_id == user_id, Booking.training_id == training_id)):
        ...
        if training and training.current_participants < training.max_participants:
            session.add(Booking(user_id=user_id, training_id=training_id, booking_date=datetime.now(), status=True))
...
async def cancel_booking(user_id, training_id):
    async with async_session() as session:
        booking = await session.scalar(
            select(Booking).where(Booking.user_id == user_id, Booking.training_id == training_id)
        )
        if booking:
...
async def get_user_bookings(user_id):
    async with async_session() as session:
        bookings = await session.scalars(select(Booking).where(Booking.user_id == user_id))
        return bookings.all()
...
async def edit_training(training_id, name=None, date=None, max_participants=None):
    async with async_session() as session:
        training = await session.get(Training, training_id)
        if not training:
            return False
...
async def get_user_bookings(user_id=None):
    async with async_session() as session:
        query = select(Booking).options(joinedload(Booking.training), joinedload(Booking.user))
        if user_id:
